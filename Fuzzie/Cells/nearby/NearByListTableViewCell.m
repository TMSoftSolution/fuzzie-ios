...
- (IBAction)likeButtonPressed:(id)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(likeButtonTappedOnList:WithState:)]) {
        if (self.isLiked) {
            [self setEmptyHeart];
        ...
        [self.delegate likeButtonTappedOnList:self.brand WithState:self.isLiked];
    }
}
...
- (void)cellTapped{
    if ([self.delegate respondsToSelector:@selector(cellTapped:)]) {
        if (self.brand) {
            [self.delegate cellTapped:self.brand];
        }
    }
}
