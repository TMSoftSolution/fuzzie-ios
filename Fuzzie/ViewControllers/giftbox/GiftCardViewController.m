...
@interface GiftCardViewController ()<UITableViewDelegate, UITableViewDataSource, AboutBrandTableViewCellDelegate, InfoBrandTableViewCellDelegate, TripAdvisorBrandTableViewCellDelegate, GiftCardRedeemTableViewCellDelegate,  FZRedeemPopViewDelegate, MarkAsUnredeemedTableViewCellDelegate>
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    } else if (section == kGiftCardSectionValid && self.brand.textOptionGiftCard.count < 1) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    if (section == kGiftCardSectionInfoConditions || section == kGiftCardSectionAboutBrand) {
        return 10;
    }
...
- (void)redeemButtonClicked{
    ...
    if (self.giftDict[@"online"] && [self.giftDict[@"online"] boolValue]) {
        ...
        NSString *redeemType = self.giftCardDict[@"redemption_type"];
    ...
    [GiftController markAsUnredeemedOnlineGift:self.giftDict[@"id"] withCompletion:^(NSDictionary *dictionary, NSError *error) {
        ...
        if (dictionary && dictionary[@"gift"]) {
            ...
            if ([self.giftDict[@"used"] boolValue]) {
                ...
                // Remove This gift from UsedGifts
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"pushToTermConditions"]) {
        if ([segue.destinationViewController isKindOfClass:[TermsAndConditionsTableViewController class]]) {
            ...
            if (self.giftDict[@"terms"] && [self.giftDict[@"terms"] isKindOfClass:[NSArray class]]) {
                termConditionView.termsConditions = self.giftDict[@"terms"];
            }
        }
    }
