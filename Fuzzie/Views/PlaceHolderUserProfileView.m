...
@implementation PlaceHolderUserProfileView
...
- (void)awakeFromNib {
    ...
    if ([FZData sharedInstance].isHomeLoading) {
        self.loader.layer.opacity = 0;
    } else{
        self.loader.layer.opacity = 1;
    }
    ...
    [UIView animateWithDuration:0.25
                     ...
                     animations:^(void) {
                         ...
                         if (mode == UserProfileModeWhishList) {
                             ...
                             weakSelf.subTitleLabel.text = @"No items available.";
                             weakSelf.iconPlaceholder.image = [UIImage imageNamed:@"bear-baby"];
                         } else {
                             weakSelf.titleLabel.text = @"";
                             weakSelf.subTitleLabel.text = @"No items available.";
                             weakSelf.iconPlaceholder.image = [UIImage imageNamed:@"bear-mustache"];
                         }
                     }
                     completion:nil];
}
